...
public class AlpacaBedRenderer implements BlockEntityRenderer<AlpacaBedBlockEntity> {
     ...
     * <p>
     * 羊驼床模型是跨两格的完整几何体，只在 FOOT 位置渲染一次。
     ...
     */
    @Override
    public @NotNull AABB getRenderBoundingBox(AlpacaBedBlockEntity blockEntity) {
        BlockState state = blockEntity.getBlockState();
        BedPart part = state.getValue(AlpacaBedBlock.PART);
        Direction facing = state.getValue(AlpacaBedBlock.FACING);
